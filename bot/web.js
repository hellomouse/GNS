...
module.exports = async function web(app) {
  ...
  router.get('/user', async (req, res) => {
    ...
    const emails = await octokit.users.getEmails();
    const orgs = await octokit.users.getOrgs();
    ...
    for await (const org of orgs) {
      ...
      try {
        ...
        if (e.name === 'not_found') {
          db.put({ ...defaults, _id: org.login })
          orgDB = await db.get(org.login);
        }
      }
    }
    res.send(
        `<p>You're logged in! Here's all your emails on GitHub: </p>${JSON.stringify(emails.data)}
            <p>Here are all your orgs that I have access to: ${JSON.stringify(orgs.data)}
